...
//!
//! Provenance: LEMMALOG_REPO names the working tree whose HEAD stamps
//! asserted facts (ADR 3; default: the current directory). It is read per
//! ingestion, not once at startup — the server outlives many commits and
//! the whole point is which one a fact was asserted at.
//!
...
use lemmalog::agent::{AgentMemory, AssertionContext, FactClass};
...
/// Working tree whose HEAD is recorded on asserted facts. Unset means the
/// current directory — the agent usually runs inside the codebase it is
/// documenting, which is rarely where the store file lives.
const REPO_ENV: &str = "LEMMALOG_REPO";

/// `(sha, branch)` of `LEMMALOG_REPO`'s HEAD, for ADR 3's provenance
/// columns. Duplicated verbatim in `lemmalog-cli.rs` for the same reason
/// the store helpers above are: the library must not learn about git, and
/// a shared module in `src/lib.rs` is the wider change (ADR 1).
///
/// `git` is asked rather than `.git/HEAD` parsed on purpose: worktrees,
/// packed refs and detached HEAD are already correct in `rev-parse` and
/// all wrong in a hand-rolled reader.
///
/// Every failure — not a repo, no git on PATH, a repo with no commits yet
/// — is `(None, None)` and silence: a store used outside a checkout keeps
/// working exactly as it did.
///
/// Detached HEAD reports branch `None`. `--abbrev-ref` answers the literal
/// "HEAD", which is not a branch name; recording it (or any invented
/// stand-in) would put a value in `edges.asserted_on_branch` that no
/// checkout can match. The sha still identifies the revision.
///
/// Cost: two `git rev-parse` forks per ingestion, single-digit
/// milliseconds — paid on `lemmalog_observe` only, never on a read.
...
/// What the ingestion was stamped with, for a caller who is not going to
/// open SQLite. `None` when there is nothing to say — no sha, no branch,
/// default class — so an observe outside a checkout reads exactly as it
...
fn prop_desc(p: &str) -> &'static str {
    match p {
        ...
        "fact_class" => "machine | agent | human (default agent)",
        _ => "",
    }
}
...
fn tool_call(state: &mut State, name: &str, args: &J, path: Option<&str>) -> Result<J, String> {
    ...
    let inner: Result<String, String> = match name {
        "lemmalog_observe" => 'observe: {
            ...
            // ADR 3: resolve HEAD per ingestion, not once at startup. This
            // server is long-lived and HEAD moves under it during a
            // session; a sha resolved at boot would label every later fact
            // with a commit it was not asserted at.
            let (sha, branch) = git_head();
            let class = args["fact_class"]
                .as_str()
                .filter(|c| !c.trim().is_empty());
            let mem = &mut state.memory;
            // An unknown class is an error naming the valid set, never a
            // silent demotion to the default.
            if let Err(e) = mem.set_assertion_context(sha.as_deref(), branch.as_deref(), class) {
                break 'observe Err(e);
            }
            ...
            if let Some(p) = provenance_line(mem.assertion_context()) {
                out.push_str(&format!("\n{p}"));
            }
